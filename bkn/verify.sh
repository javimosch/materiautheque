...
# admin actions of the suite carry register:"skip": the live register must never hold test objects.
# The register path itself is covered by greffe's own suites and by the first real approval.
apost() { curl -s -H 'Content-Type: application/json' -H "X-Admin-Token: $MAT_ADMIN_TOKEN" -X POST "$H" -d "$(echo "$1" | sed 's/^{/{"register":"skip",/')"; }
...
chk "approval reports the register step as skipped (test run)" "$(echo "$AP" | j "'skipped' in d['register']")" "True"
chk "admin edit of the nickname reaches the register step" "$(apost "{\"action\":\"edit\",\"id\":\"$IID\",\"fields\":{\"nickname\":\"Testeur\"}}" | j "'skipped' in d['register']")" "True"
chk "the nickname is public" "$(curl -s "$H" | j "[i['nickname'] for i in d['items'] if i['id']=='$IID'][0]")" "Testeur"
chk "status change to 'parti' reaches the register step" "$(apost "{\"action\":\"status\",\"id\":\"$IID\",\"status\":\"parti\"}" | j "'skipped' in d['register']")" "True"
